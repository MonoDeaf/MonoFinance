...
export function updateStreakUI(updateDashboardCallback) {
    ...
    const isLightMode = financeState.profile?.theme === 'light';
    const cardBg = isLightMode ? 'bg-[var(--bg-card)] text-[var(--text-main)] border border-[var(--border-main)]' : 'bg-gradient-to-b from-[#1a1a1a] to-[#0d0d0d] border border-white/5';
    const accentColor = 'var(--accent-primary)';

    if (financeState.dailyUpdateViewActive && !isUpdatedToday) {
        container.innerHTML = `
            <div class="${cardBg} ${isLightMode ? '' : 'border-[#ccccfa]/20'} rounded-2xl p-5 relative">
                <div class="flex justify-between items-center mb-4">
                    <h4 class="text-xs font-bold uppercase tracking-widest text-[var(--text-main)]">Daily Log</h4>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <label class="cursor-pointer">
                                ...
                                <div class="py-2 text-[10px] text-center border border-white/5 rounded-lg bg-white/5 peer-checked:border-[var(--accent-primary)] peer-checked:bg-[var(--accent-primary)]/10 transition-all font-bold">Entry</div>
                            </label>
                            <label class="cursor-pointer">
                                <input type="radio" name="updateType" value="no-change" class="hidden peer">
                                <div class="py-2 text-[10px] text-center border border-white/5 rounded-lg bg-white/5 peer-checked:border-[var(--accent-primary)] peer-checked:bg-[var(--accent-primary)]/10 transition-all font-bold">Check-in</div>
                            </label>
                        </div>
                    </div>
        ...
        container.innerHTML = `
            <div class="${cardBg} rounded-2xl p-5 relative overflow-hidden group">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex flex-col">
                        <span class="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] mb-1">Daily Streak</span>
                        <div class="flex items-baseline gap-2">
                            <h4 class="text-3xl font-black text-[var(--text-main)]">${streak}</h4>
                            <span class="text-xs text-gray-500 font-medium">days</span>
                        </div>
                    </div>
                    <div class="w-10 h-10 ${isUpdatedToday ? 'bg-[var(--accent-primary)]/20 text-[var(--accent-primary)]' : 'bg-white/5 text-gray-600'} rounded-xl flex items-center justify-center transition-colors">
                ...
                <button onclick="${isUpdatedToday ? '' : 'window.openDailyUpdateForm()'}" class="w-full py-3 ${isUpdatedToday ? 'bg-white/5 text-gray-400 cursor-default' : 'bg-[var(--accent-primary)] text-[var(--btn-text)] hover:opacity-80'} text-[11px] font-bold rounded-xl transition-all flex items-center justify-center gap-2">
